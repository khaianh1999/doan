...
@include('admin.phones.search', ['route' => route('admin.phones.index')])
<form method="post">
    @csrf
    @method('POST')
    <div class="action-list col-12 mb-2" style="display:flex; justify-content:space-between; align-items: center;">
        <div class="amount-user">
            @if ($phones->total() <= 0) @else {!! $phones->total() !!} Điện thoại
                @endif
        </div>
        <div class="text-right">
            <a href="{{ route('admin.phones.create') }}" class="text-success">Tạo mới</a>
        </div>
    </div>
                    ...
                    <tbody>
                        @if ($phones->total() <= 0) <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('admin.phones.destroy', $item->id) }}" method="POST" style="float: left; margin-right: 7px">
                                        <a class="btn btn-primary btn-sm mr-1" href="{{ route('admin.phones.edit', $item->id) }}"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-danger btn-sm delete-d" id="{{ $item->id }}" data="phones"><i class="fas fa-trash"></i></a>
                                    </form>
                                    ...
                                    <div id="{{ $item->id }}" class="status" data="phones">
                                        @if ($item->status === 1)
...
<div class="row">
    @if ($phones->total() <= 0) <p>
        ...
        @else
        <div class="col-6">
            Hiển thị : {!! $phones->currentPage() * $phones->perPage() - $phones->perPage() + 1 !!} - {!! $phones->currentPage() * $phones->perPage() - $phones->perPage() + $phones->count() !!}
            của {!! $phones->total() !!}
        </div>
        <div class="col-6">
            <div class="float-right">
                {!! $phones->appends(request()->except(['page', '_token']))->links() !!}
            </div>
        </div>
        @endif
</div>
