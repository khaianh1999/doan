...
@include('admin.laptops.search', ['route' => route('admin.laptops.index')])
<form method="post">
    @csrf
    @method('POST')
    <div class="action-list col-12 mb-2" style="display:flex; justify-content:space-between; align-items: center;">
        <div class="amount-user">
            @if ($laptops->total() <= 0) @else {!! $laptops->total() !!} Laptop
                @endif
        </div>
        <div class="text-right">
            <a href="{{ route('admin.laptops.create') }}" class="text-success">Tạo mới</a>
        </div>
    </div>
                    ...
                    <tbody>
                        @if ($laptops->total() <= 0) <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('admin.laptops.destroy', $item->id) }}" method="POST" style="float: left; margin-right: 7px">
                                        <a class="btn btn-primary btn-sm mr-1" href="{{ route('admin.laptops.edit', $item->id) }}"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="javascript:void(0)" class="btn btn-danger btn-sm delete-d" id="{{ $item->id }}" data="laptops"><i class="fas fa-trash"></i></a>
                                    </form>
                                    ...
                                    <div id="{{ $item->id }}" class="status" data="laptops">
                                        @if ($item->status === 1)
...
<div class="row">
    @if ($laptops->total() <= 0) <p>
        </p>
        @else
        <div class="col-6">
            Hiển thị : {!! $laptops->currentPage() * $laptops->perPage() - $laptops->perPage() + 1 !!} - {!! $laptops->currentPage() * $laptops->perPage() - $laptops->perPage() + $laptops->count() !!}
            của {!! $laptops->total() !!}
        </div>
        <div class="col-6">
            <div class="float-right">
                {!! $laptops->appends(request()->except(['page', '_token']))->links() !!}
            </div>
        </div>
        @endif
</div>
